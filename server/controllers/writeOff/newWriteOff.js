...
router.post("/", jsonParser, (req, res) => {
    ...
    database.query(`SELECT * FROM suppliers WHERE company_name = $1`, [supplier], (err, result) => {
        ...
        if(result.rows.length > 0) {
            ...
            database.query(`INSERT INTO write_off (supplier_id, item_name, writeoff_id, quantity, units, date) VALUES ($1, $2, $3, $4, $5, $6);`, [supplierId, itemName, postID, quantity, units, date], (err, result) => {
                if(err) {
                    console.log(err);
                    res.send({error: err});
                } else {
